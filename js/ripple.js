/* ============================================================
   ...
   fragment shader (domain-warped fbm). Monochrome navy/steel,
   near-invisible at rest, a gentle swell follows the cursor.
   No concentric rings. Frame-capped, low-res backing store.
   Reduced-motion / no-WebGL → static CSS gradient + grain.
   Exposes window.RIPPLE.setAudioLevel(0..1) and .burst(x,y).
   ============================================================ */
(function () {
  ...
  function staticFallback() {
    ...
    window.RIPPLE = { setAudioLevel() {}, burst() {} };
  }
  if (reduce || !canvas) { staticFallback(); return; }

  const gl = canvas.getContext('webgl', { antialias: false, depth: false, alpha: false, premultipliedAlpha: false, powerPreference: 'low-power' });
  ...
  void main(){
    ...
    // faint caustic glints only where the field folds
  ...
  if (!vs || !fs) { staticFallback(); return; }
  const prog = gl.createProgram(); gl.attachShader(prog, vs); gl.attachShader(prog, fs); gl.linkProgram(prog);
  if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) { staticFallback(); return; }
  gl.useProgram(prog);
  ...
  addEventListener('pointermove', (e) => {
    ...
    tmx = e.clientX * SCALE; tmy = (innerHeight - e.clientY) * SCALE;
    str = Math.min(1, str + 0.12);
  }, { passive: true });
  addEventListener('pointerdown', (e) => { str = 1; });

  window.RIPPLE = {
    setAudioLevel(v) { audio = Math.max(0, Math.min(1, v)); },
    burst(x, y) {
      if (typeof x === 'number') { tmx = x * SCALE; tmy = (innerHeight - y) * SCALE; }
      str = 1.0;
    }
  };

  const FRAME = 1000 / (lowPower ? 30 : 45);
  let last = performance.now(), acc = 0, t0 = last;
  function draw(now) {
    const dt = now - last; last = now; acc += dt;
    if (acc < FRAME) { requestAnimationFrame(draw); return; }
    acc = 0;
